...
struct PatternFiller {
  ...
  virtual OpSet fillPolygon(const std::vector<Point>& points, const Options& o) = 0;
};
...
struct RenderHelper {
  ...
  virtual std::vector<Op> doubleLineOps(double x1, double y1, double x2, double y2, const Options& o) = 0;
  
  std::vector<Op> doubleLineOps(const Point& p1, const Point& p2, const Options& o);
  std::vector<Op> doubleLineOps(const Line& line, const Options& o);
};
