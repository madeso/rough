...
OpSet DashedFiller::fillPolygon(const std::vector<Point>& points, ResolvedOptions& o) {
  const auto lines = polygonHachureLines(points, o);
  return OpSet{ OpSetType::fillSketch, this->dashedLine(lines, o) };
}

std::vector<Op> DashedFiller::dashedLine(const std::vector<Line>& lines, ResolvedOptions& o) {
