...
OpSet HachureFiller::fillPolygon(const std::vector<Point>& points, ResolvedOptions& o) {
  return this->_fillPolygon(points, o);
}

OpSet HachureFiller::_fillPolygon(const std::vector<Point>& points, ResolvedOptions& o, bool connectEnds) {
...
std::vector<Op> HachureFiller::renderLines(const std::vector<Line>& lines, ResolvedOptions& o) {
  std::vector<Op> ops;
