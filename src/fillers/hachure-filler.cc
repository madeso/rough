...
OpSet HachureFiller::fillPolygon(const std::vector<Point>& points, const ResolvedOptions& o) {
  return this->_fillPolygon(points, o);
}

OpSet HachureFiller::_fillPolygon(const std::vector<Point>& points, const ResolvedOptions& o, bool connectEnds) {
...
std::vector<Op> HachureFiller::renderLines(const std::vector<Line>& lines, const ResolvedOptions& o) {
  std::vector<Op> ops;
