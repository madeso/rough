...
struct HachureFiller : PatternFiller {
  ...
  virtual OpSet fillPolygon(const std::vector<Point>& points, ResolvedOptions& o);
  OpSet _fillPolygon(const std::vector<Point>& points, ResolvedOptions& o, bool connectEnds= false);
  std::vector<Op> renderLines(const std::vector<Line>& lines, ResolvedOptions& o);
